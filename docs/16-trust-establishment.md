...
**The genuinely new contribution is the STRUCTURAL UNCONFLATION of access from reputation enforced as runtime code:** **Standing-`R` has, by construction, NO interface method that returns a privilege grant** (only a capped `friction_discount`), so "reputation raises privilege" is not a policy one must remember to forbid but a capability that **does not exist in the type system**. Wrapped around that are four reused-but-newly-wired pieces:
