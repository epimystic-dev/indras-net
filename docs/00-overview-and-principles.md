...
- a handful of genuinely SOTA-advancing integration moves that no surveyed system makes (each detailed in its subsystem doc):
  - **corrigibility as a structural property of the agent model** (the agent as an ephemeral "actual occasion" whose persistent identity is owned data it cannot defend — §07's agent model);
