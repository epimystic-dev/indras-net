...
**Net assessment:** counted by the rows above, the clear majority of the reference components (the nine marked "Yes"/"Yes — mature") are assembly of mature, individually-validated parts; the residual minority (witness governance, the coordination bus, the archive selection-term) carry the calibration-open or partly-open work. This split is **by component count, not by risk weight** — that residual minority is the safety-critical surface (the set-points and witness/selection contracts whose miscalibration is exactly the failure mode), so it carries disproportionate risk relative to its share of the rows. The novelty — and the residual risk — is in the **wiring and the contracts between layers**, plus the calibration-open set-points that are quarantined behind shadow-mode. We do **not** claim the *composed* loop is validated end-to-end; no one has yet run this integrated cell continuously against an adaptive multi-agent red team (§08 open problem).
...
**The bottom line, stated at its true rung.** This roadmap is a coherent, buildable **reference design** for the first integration of safe self-evolution + homeostasis + principled governance + anti-collusion into one cell. By the §1.1 row count, most of the stack is mature assembly; the residual minority that is novel is the *wiring and the sequencing* — and that minority, though small by component count, is the safety-critical minority. The hardest safety properties are **empirical mitigations under a fail-safe-asymmetric, append-only-tightening discipline — not proofs**. Built in this order, safety-erosion-under-self-evolution is made *structurally hard*. It is not made *impossible*. The honest claim is the smaller one, and making the smaller claim — refusing to present rung-2 interventional reasoning as a rung-3 structural guarantee — is itself the architecture's first law, enforced here in its own documentation.
